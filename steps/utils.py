...
def _get_ml_config_value(env, key):
    ...
    conf_file_path = os.path.join(os.pardir, "databricks-config", "output", f"{env}.json")
    try:
        with open(conf_file_path, "r") as handle:
            data = json.loads(handle.read())
    except FileNotFoundError as e:
        raise RuntimeError(f"Unable to find file '{conf_file_path}'. Make sure ML config-as-code resources defined "
                           f"under databricks-config have been deployed to {env} (see databricks-config/README in the "
                           f"current git repo for details)") from e
    try:
        return data[key]["value"]
    except KeyError as e:
        raise RuntimeError(f"Unable to load key '{key}' for env '{env}'. Ensure that key '{key}' is defined "
                           f"in f{conf_file_path}") from e
...
def get_model_name(env, test_mode=False):
    ...
    if env == 'dev' or test_mode:
        ...
        return f"mlops-demo-classifier-model{resource_name_suffix}"
    else:
        # Read ml model name from databricks-config
        return _get_ml_config_value(env, "mlops-demo-classifier_model_name")
...
def set_experiment(env, test_mode=False):
    ...
    if env == 'dev':
        ...
        # to eliminate the need to create a directory named '/Users/ryuta.yoshimatsu@databricks.com/mlops-demo-classifier'
        # to run ML code.
        experiment_name = f"/mlops-demo-classifier-experiment{resource_name_suffix}"
    elif test_mode:
        experiment_name =\
            f"/Users/ryuta.yoshimatsu@databricks.com/mlops-demo-classifier/mlops-demo-classifier-experiment{resource_name_suffix}"
    else:
        # Read mlflow experiment name from databricks-config
        experiment_name = _get_ml_config_value(env, "mlops-demo-classifier_experiment_name")

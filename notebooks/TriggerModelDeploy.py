...
github_repo = dbutils.secrets.get(f"{env}-mlops-demo-classifier-cd-credentials", "github_repo")
token = dbutils.secrets.get(f"{env}-mlops-demo-classifier-cd-credentials", "token")
...
assert response.ok, f"Triggering CD workflow {cd_trigger_url} for model {model_uri} " \
                    f"failed with status code {response.status_code}. Response body:\n{response.content}"

# COMMAND ----------
print(f"Successfully triggered model CD deployment workflow for {model_uri}. See your CD provider to check the "
